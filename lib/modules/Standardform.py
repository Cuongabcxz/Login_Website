...
class Standardform:
    """
    Mô-đun này cố gắng tự động phát hiện các biểu mẫu xác thực web tiêu chuẩn và thực thi chúng.
    Nó dựa trên một số phương pháp phỏng đoán, và do đó có thể dẫn đến kết quả positives/negatives to false.
    Nó không xử lý các biểu mẫu dựa trên javascript hoặc các biểu mẫu xác thực kỳ lạ.
    """
    ...
    def __find_username_field_via_name(self, inputs):
        """
        Cố gắng tìm tên của trường tên người dùng trong danh sách các trường đầu vào.
        Tìm kiếm giá trị gợi nhiều nhất cho thuộc tính "name"
        : đầu vào danh sách tham số: Danh sách các ten đầu vào
        : return str name: Tên trường tên người dùng (Không có nếu không tìm thấy)
        """
        for input in inputs:
